...
export const postReply = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (parentReplyId) {
      ...
      parentReplyAuthorId = String((parent as any)?.authorId ?? "");
    }
    ...
    if (parentReplyAuthorId && parentReplyAuthorId !== actorId) recipientIds.add(parentReplyAuthorId);
...
export const upvoteReply = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const r: any = await Reply.findOne({ _id: replyId, questionId, answerId });
    if (!r) return res.status(404).json({ message: "Reply not found" });

    const userId = String(req.user.id);
    const idx = r.voters.findIndex((v: any) => String(v.userId) === userId);
    const existing = idx >= 0 ? Number(r.voters[idx].value) : null;

    const { newValue, delta } = applyVote(existing, 1);

    if (newValue === null) {
      r.voters = r.voters.filter((v: any) => String(v.userId) !== userId);
    } else if (idx >= 0) {
