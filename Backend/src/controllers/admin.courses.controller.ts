...
async function buildAdminCourseRows(filter: Record<string, unknown>) {
  ...
  const instructorIds = items.map((item: any) => item.instructorId);
  const instructors = await User.find({ _id: { $in: instructorIds } })
    .select("firstName lastName email")
    .lean();

  const instructorMap = new Map<string, any>();
  instructors.forEach((user: any) => instructorMap.set(String(user._id), user));
...
export async function listAdminCourses(req: AuthedRequest, res: Response) {
  try {
...
export async function listPendingCourses(_req: AuthedRequest, res: Response) {
  try {
