...
const parseRequestedInterestTags = (value: unknown): string[] => {
...
const tagMatches = (candidateTag: string, requestedTag: string): boolean => {
...
const hasCurrentAcademicBackground = (academicBackgrounds: unknown): boolean =>
  Array.isArray(academicBackgrounds) &&
  academicBackgrounds.some(
    (item) => Boolean((item as any)?.isCurrent) && String((item as any)?.institution ?? "").trim()
  );

const getPairKey = (a: string, b: string) => [String(a), String(b)].sort().join(":");

const isEligibleMentorCandidate = (candidate: any): boolean => {
  const role = String(candidate?.role ?? "");
  return role === "instructor" || role === "student";
};

const resolveInstitution = (candidate: any): string => {
  const direct = String(candidate?.institution ?? "").trim();
  if (direct) return direct;
  if (!Array.isArray(candidate?.academicBackgrounds)) return "";
  const firstAcademic = candidate.academicBackgrounds.find((item: any) =>
    String(item?.institution ?? "").trim()
  );
  return firstAcademic ? String(firstAcademic.institution).trim() : "";
};

const toMentorPayload = (candidate: any, scoreBreakdown: any, totalScore: number) => {
  const fullName = `${String(candidate?.firstName ?? "").trim()} ${String(
    candidate?.lastName ?? ""
  ).trim()}`.trim();
...
const getPeerIdFromConnection = (connection: any, currentUserId: string): string =>
...
export async function getNextMentorMatch(req: AuthedRequest, res: Response) {
  try {
    ...
    const requestedInterestTags = parseRequestedInterestTags((req.query as any)?.tags);
    ...
    actions.forEach((item: any) => {
      ...
      if (String(item.action) === "Blocked") blockedIds.add(mentorId);
      if (String(item.action) === "Skipped") skippedIds.add(mentorId);
    });
    activeConnections.forEach((item: any) =>
      connectedIds.add(getPeerIdFromConnection(item, currentUserId))
    );
    ...
    const buildRankedCandidates = async (excludedUserIds: Set<string>) => {
      const candidates = await User.find({
        _id: { $nin: Array.from(excludedUserIds) },
        role: { $in: ["student", "instructor"] },
      })
        .select(
          "_id firstName lastName email role institution expertise interests bio avatarUrl academicBackgrounds isVerified verificationStatus points acceptedAnswers"
        )
        .lean();

      const eligibleCandidates = candidates.filter((candidate) =>
        isEligibleMentorCandidate(candidate)
      );

      return eligibleCandidates.map((candidate) => {
        const candidateInterests = sanitizeStringArray((candidate as any).interests);
        const sharedInterestSet = new Set<string>();
        const sharedInterests = candidateInterests.filter((item) => {
          const normalized = normalizeText(item);
          if (sharedInterestSet.has(normalized)) return false;
          const matchesRequested = requestedInterestTags.some((requestedTag) =>
            tagMatches(item, requestedTag)
          );
          if (!matchesRequested) return false;
          sharedInterestSet.add(normalized);
          return true;
        });

        const matchingInterestCount = sharedInterests.length;
        const currentAcademic = hasCurrentAcademicBackground((candidate as any).academicBackgrounds);
        const points = Number((candidate as any)?.points ?? 0);
    ...
    if (!matched.length && skippedIds.size > 0) {
      const excludedWithoutSkipped = new Set<string>([
        currentUserId,
        ...Array.from(blockedIds),
        ...Array.from(connectedIds),
      ]);
    ...
    if (matched.length > 0) {
      ...
      const top = matched[0];
      const equallyTop = matched.filter(
        (item) =>
          item.matchingInterestCount === top.matchingInterestCount &&
      ...
      return res.json({
        mentor: toMentorPayload(
          selected.candidate,
          selected.breakdown,
          selected.matchingInterestCount
        ),
      });
    }
    ...
    const selected =
      randomFallbackPool[Math.floor(Math.random() * randomFallbackPool.length)];
...
export async function skipMentor(req: AuthedRequest, res: Response) {
  try {
    const currentUserId = String(req.user?.id || "").trim();
    if (!currentUserId) return res.status(401).json({ message: "Unauthorized" });
    ...
    await MentorDiscoveryAction.updateOne(
      { userId: currentUserId, mentorId, action: "Skipped" },
      {
        $setOnInsert: {
...
export async function blockMentor(req: AuthedRequest, res: Response) {
  try {
    const currentUserId = String(req.user?.id || "").trim();
    if (!currentUserId) return res.status(401).json({ message: "Unauthorized" });
    ...
    await MentorDiscoveryAction.updateOne(
      { userId: currentUserId, mentorId, action: "Blocked" },
      {
        $setOnInsert: {
          userId: new mongoose.Types.ObjectId(currentUserId),
          mentorId: new mongoose.Types.ObjectId(mentorId),
    ...
    await MentorConnection.updateOne(
      { pairKey, status: { $in: ["Pending", "Accepted"] } },
      {
        $set: {
          status: "Cancelled",
...
export async function connectWithMentor(req: AuthedRequest, res: Response) {
  try {
    const currentUserId = String(req.user?.id || "").trim();
    if (!currentUserId) return res.status(401).json({ message: "Unauthorized" });
    ...
    if (connection.status === "Accepted") {
      const room = await ensurePrivateChatRoom(
        currentUserId,
        mentorId,
        String(connection._id)
      );
      ...
      await createNotification({
        ...
        link: "/mentors",
        metadata: { connectionId: String(connection._id), chatRoomId: room ? String((room as any)._id) : null },
      });

      return res.json({
        connectionId: String(connection._id),
        status: connection.status,
        chatRoomId: room ? String((room as any)._id) : null,
...
export async function respondToConnectionRequest(req: AuthedRequest, res: Response) {
  try {
    const currentUserId = String(req.user?.id || "").trim();
    if (!currentUserId) return res.status(401).json({ message: "Unauthorized" });
