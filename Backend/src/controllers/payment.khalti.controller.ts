...
export async function verifyKhaltiPayment(req: Request, res: Response) {
  ...
  try {
    ...
    const status = String((lookup.data as Record<string, unknown>)?.status ?? "");
    if (status !== "Completed") {
      return res.status(400).json({
        success: false,
    ...
    const baseDate =
      existingExpiry && !Number.isNaN(existingExpiry.getTime()) && existingExpiry.getTime() > now.getTime()
        ? existingExpiry
        : now;
...
export async function initiateKhaltiPayment(req: Request, res: Response) {
  ...
  const rawCustomerInfo =
    customerInfoRaw && typeof customerInfoRaw === "object"
      ? {
          name: String((customerInfoRaw as Record<string, unknown>).name ?? "").trim(),
          email: String((customerInfoRaw as Record<string, unknown>).email ?? "").trim(),
          phone: String((customerInfoRaw as Record<string, unknown>).phone ?? "").trim(),
        }
      : null;

  const customerInfo =
    rawCustomerInfo && (rawCustomerInfo.name || rawCustomerInfo.email || rawCustomerInfo.phone)
      ? {
