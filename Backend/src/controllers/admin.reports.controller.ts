...
const toNpr = (paisa: number) => Number((paisa / 100).toFixed(2));
...
async function dailyRevenueMap(params: { start: Date; endExclusive: Date }) {
...
const revenueInRangePaisa = async (start: Date, endExclusive: Date) => {
  const rows = await CoursePurchase.aggregate([
    {
      $match: {
        status: "completed",
...
export async function getAdminReportInsights(req: Request, res: Response) {
  try {
    ...
    const [
      ...
      totalRevenueAgg,
      usersInWindow,
      usersInPreviousWindow,
      questionsInWindow,
      questionsInPreviousWindow,
      answersInWindow,
      answersInPreviousWindow,
      enrollmentsInWindow,
      enrollmentsInPreviousWindow,
      purchasesInWindow,
      purchasesInPreviousWindow,
      revenueInWindowPaisa,
      revenueInPreviousWindowPaisa,
    ] = await Promise.all([
      User.countDocuments({}),
    ...
    const [enrollmentByCourseRows, purchaseByCourseRows, reviewByCourseRows, publishedByInstructorRows] =
      await Promise.all([
    ...
    const ensureCourseMetric = (courseId: string): CourseMetric => {
      const existing = metricsByCourse.get(courseId);
      if (existing) return existing;
      const fresh: CourseMetric = {
        enrollments: 0,
        purchases: 0,
        revenuePaisa: 0,
        averageRating: 0,
    ...
    const publishedCourseDocs = courses.filter((c: any) => c.status === "Published");
    const instructorIdsFromCourses = Array.from(
      new Set(publishedCourseDocs.map((c: any) => String(c.instructorId || "")).filter(Boolean))
    );

    const instructorIdsFromPublishedCount = Array.from(
      new Set(publishedByInstructorRows.map((r) => String(r._id || "")).filter(Boolean))
    );
    ...
    const instructorMap = new Map<string, { fullName: string; email: string }>();
    for (const ins of instructors as Array<Record<string, unknown>>) {
    ...
    const topCourses = publishedCourseDocs
      ...
      .sort((a, b) => {
        if (b.revenueNpr !== a.revenueNpr) return b.revenueNpr - a.revenueNpr;
    ...
    const instructorMetrics = new Map<string, InstructorMetric>();
    const ensureInstructorMetric = (id: string): InstructorMetric => {
      const existing = instructorMetrics.get(id);
      if (existing) return existing;
      const fresh: InstructorMetric = {
        publishedCourses: 0,
        enrollments: 0,
        purchases: 0,
        revenuePaisa: 0,
    ...
    const recentPurchases = await CoursePurchase.find({ status: "completed" })
      .select(
        "_id pidx userId instructorId courseId paidAmountPaisa pricePaisa platformFeePaisa instructorSharePaisa createdAt"
      )
      .sort({ createdAt: -1 })
      .limit(12)
      .lean();

    const txUserIds = Array.from(
      new Set(
        recentPurchases
          .flatMap((tx: any) => [String(tx.userId || ""), String(tx.instructorId || "")])
          .filter(Boolean)
      )
    );
    const txCourseIds = Array.from(new Set(recentPurchases.map((tx: any) => String(tx.courseId || "")).filter(Boolean)));
    ...
    const txUserMap = new Map<string, { name: string; email: string }>();
    for (const u of txUsers as Array<Record<string, unknown>>) {
    ...
    const txCourseMap = new Map<string, string>();
    for (const c of txCourses as Array<Record<string, unknown>>) {
      const id = String(c._id || "");
      if (!id) continue;
      txCourseMap.set(id, String(c.title || "Untitled course"));
    }

    const revenueTransactions = recentPurchases.map((tx: any) => {
    ...
    if (pendingTotal > 0) {
      insights.push(
        `${pendingTotal} admin item(s) are pending review (${pendingCourseApprovals} course approvals, ${pendingInstructorVerifications} instructor verifications).`
      );
    } else {
    ...
    if (unansweredQuestions > 0) {
      insights.push(
        `${unansweredQuestions} question(s) are unanswered. Current answered-question rate is ${answeredQuestionRate}%.`
      );
    }
